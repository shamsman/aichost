...
<div class="container" style="padding-top: 32px; padding-bottom: 80px;">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 30px;">
        <div>
            <h1 style="font-size: 28px; font-weight: 800; color: #fff;">Invoices & Billing History</h1>
    ...
    @if($invoices->isEmpty())
        <div class="glass-panel" style="padding: 60px; text-align: center;">
            ...
            <h3 style="font-size: 18px; margin-bottom: 8px;">No invoices generated</h3>
                    ...
                    @foreach($invoices as $inv)
                        <tr style="border-bottom: 1px solid rgba(255,255,255,0.04);">
                            <td style="padding: 16px;">
                                <strong style="color: #fff; font-family: 'Space Grotesk', sans-serif;">{{ $inv->invoice_number }}</strong>
                            </td>
                            <td style="padding: 16px; color: var(--text-muted);">
                                {{ $inv->paid_at?->format('M d, Y') ?? $inv->created_at->format('M d, Y') }}
                            </td>
                            <td style="padding: 16px;">
                                <strong style="color: #fff;">${{ number_format($inv->total, 2) }} {{ $inv->currency }}</strong>
                            </td>
                            <td style="padding: 16px;">
                                <span style="background: rgba(16, 185, 129, 0.15); color: #34d399; font-size: 11px; font-weight: 700; padding: 3px 10px; border-radius: 12px; text-transform: uppercase;">
                                    {{ $inv->status }}
                                </span>
                            </td>
